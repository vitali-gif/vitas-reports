/**
 * /api/cron/prefetch-crm — BMBY (CRM) only (runs in ~30-40s, under 60s limit)
 * Runs at 07:10 + 14:10 Israel time — 10 min after prefetch-ads to avoid load spikes.
 * BMBY is a slow SOAP endpoint; keeping it separate ensures it always completes.
 */
...
export async function GET(request) {
  ...
  const sources = ['bmby']  // CRM ONLY — Ads handled by prefetch-ads
  const jobs = []

  // Monthly first (most important for clients)
  ...
  // CONCURRENCY=3 — BMBY SOAP endpoint; keep gentle to avoid timeouts
  const CONCURRENCY = 3
  const queue = [...jobs]
