...
async function runSync(opts = {}) {
  ...
  const projectResults = await Promise.all(projectsList.map(async (p) => {
    ...
    totals.leadsToHandle = _noResponseCids.size  // "לא טופלו" = period leads with no human follow-up (rep hasn't contacted). (client_stage has no 'not_handeled' value; no-response is the real signal)
    ...
    const CRM_SCHEMA_VERSION = 30  // v15: livingStatus + propertyType per crmRepRow (מצב דיור / סוג נכס — HI PARK)
