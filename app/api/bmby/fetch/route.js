  ...
  const projectResults = await Promise.all(projectsList.map(async (p) => {
    ...
    if (opts.cfDate) {
      ...
      const rows = todays.map(c => ({
        name: c.fname||'', client_id: c.client_id, client_date: c.client_date,
        media: c.media||'', platform: (c._cf&&c._cf['פלטפורמת פרסום'])||'', campaign: (c._cf&&c._cf['שם קמפיין'])||'',
        adset: (c._cf&&c._cf['שם סדרת מודעות'])||'', ad: (c._cf&&c._cf['שם מודעה'])||'', propertyType: (c._cf&&c._cf['סוג נכס'])||''
      }))
      const fb = rows.filter(r => /facebook|פייסבוק|fb/i.test(r.media))
      const withPlat = rows.filter(r=>r.platform).length
      return { project: p.name, cfDate: day, totalToday: rows.length, fbToday: fb.length, withPlatform: withPlat, rows: rows.slice(0,30) }
    }
    if (_stagesOnly) {
