...
export default function AdminPage() {
  ...
  const renderDashboard = useCallback(() => {
    ...
    const buildTable = (items, prevItems, labelName, tableId) => {
      ...
      const cellBg = (key, val) => { const e = extremes[key]; if (!e || val <= 0 || e.min === e.max) return {}; const col = cols.find(c=>c.key===key); if (!col || col.higher === undefined) return {}; if (val === e.max) return col.higher ? {background:'rgba(16,185,129,0.13)',color:'#059669',fontWeight:700} : {background:'rgba(239,68,68,0.13)',color:'#dc2626',fontWeight:700}; if (val === e.min) return col.higher ? {background:'rgba(239,68,68,0.13)',color:'#dc2626',fontWeight:700} : {background:'rgba(16,185,129,0.13)',color:'#059669',fontWeight:700}; return {}; };
