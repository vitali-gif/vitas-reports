 ...
 */
export async function fetchGoogleDaily(customerId, since, until, { signal } = {}) {
  const creds = credsFor(customerId)
  const gopts = { login: creds.loginCustomerId, developerToken: creds.developerToken, ...(signal ? { signal } : {}) }
