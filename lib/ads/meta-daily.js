...
async function statusMaps(accountId, token, signal) {
  ...
  const [cs, as, ads] = await Promise.allSettled([
    metaFetchAll(`${base}/campaigns?fields=id,effective_status,status&limit=500`, token, { signal }),
    metaFetchAll(`${base}/adsets?fields=id,effective_status,status&limit=500`, token, { signal }),
    // מודעות פעילות בלבד, עם גוף המודעה — אותו סינון כמו ב-meta/fetch, כי creative{} יקר.
    metaFetchAll(`${base}/ads?fields=id,effective_status,status,creative{body,object_story_spec}&effective_status=${encodeURIComponent(JSON.stringify(['ACTIVE']))}&limit=50`, token, { signal }),
  ])
...
/**
 * @param {{signal?: AbortSignal}} [opts] — signal קוטע את הדפדוף כשתקציב הזמן של הקורא נגמר.
 * @returns {Promise<{rows: object[], diag: object}>} שורות בצורת ad_daily (source='facebook')
 */
export async function fetchMetaDaily(accountId, since, until, { signal } = {}) {
  ...
  const [insights, maps] = await Promise.all([metaFetchAll(url, token, { signal }), statusMaps(accountId, token, signal)])
