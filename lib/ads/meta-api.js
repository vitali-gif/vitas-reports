...
/**
 * דפדוף מלא על תשובת Graph API.
 *
 * ⚠️ maxPages זורק, לא עוצר בשקט. עד 22.9.2026 התקרה הייתה 50 עמודים והלולאה פשוט
 * יצאה והחזירה את מה שהספיקה — בלי שגיאה ובלי סימן. syncOne מוחק את כל הטווח לפני
 * שהוא כותב, ולכן משיכה שנקטעה באמצע *מחקה* את הימים שלא הספיקו לחזור. ככה נוצרו
 * חורים של 13 ו-19 ימים בחשבון של BCureLaser (ראו מיגרציה 021). תשובה חלקית שנראית
 * כמו תשובה מלאה היא הדבר היחיד שאסור להחזיר כאן.
 */
export async function metaFetchAll(url, token, { signal, maxPages = 400 } = {}) {
  const out = []
  let next = url
  let pages = 0
  while (next) {
    if (pages >= maxPages) {
      throw new Error(`Meta API pagination exceeded ${maxPages} pages (${out.length} rows so far) — refusing to return partial data`)
    }
    const sep = next.includes('?') ? '&' : '?'
    const full = next.includes('access_token=') ? next : `${next}${sep}access_token=${encodeURIComponent(token)}`
    // signal (אופציונלי): מאפשר לקורא לקטוע דפדוף ארוך כשתקציב הזמן שלו נגמר (daily-sync).
    const res = await fetch(full, signal ? { signal } : undefined)
    if (!res.ok) {
