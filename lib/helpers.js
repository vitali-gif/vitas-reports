...
export function findCol(row, possibleNames) {
  for (const key of Object.keys(row)) {
    ...
    for (const name of possibleNames) {
      if (lk === name.toLowerCase().trim()) return row[key];
    }
  }
  return '';
}
...
export function mapFacebookRows(jsonRows) {
  return jsonRows.map(row => ({
    campaign: findCol(row, ['campaign name', 'campaign', 'שם קמפיין', 'Campaign name']),
    adSet: findCol(row, ['ad set name', 'ad set', 'שם קבוצת מודעות', 'Ad set name', 'Ad Set Name']),
    adName: findCol(row, ['ad name', 'ad', 'שם מודעה', 'Ad name', 'Ad Name']),
    adText: findCol(row, ['body', 'ad body', 'body (dynamic creative)', 'Body', 'Body (Dynamic Creative)', 'Link message']),
    gender: findCol(row, ['gender', 'מגדר', 'Gender']),
    age: findCol(row, ['age', 'גיל', 'Age']),
    spend: findCol(row, ['amount spent (ils)', 'amount spent', 'spend', 'cost', 'Amount spent (ILS)', 'Amount Spent (ILS)', 'Amount spent']),
    impressions: findCol(row, ['impressions', 'חשיפות', 'Impressions']),
    reach: findCol(row, ['reach', 'חשיפה ייחודית', 'Reach']),
    clicks: findCol(row, ['link clicks', 'clicks (all)', 'clicks', 'Link clicks', 'Clicks (all)', 'Link Clicks']),
    leads: findCol(row, ['leads', 'results', 'לידים', 'Leads', 'Results', 'on-facebook leads', 'On-Facebook Leads', 'On-facebook leads']),
