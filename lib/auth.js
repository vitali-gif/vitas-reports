 ...
 *   2. אדמין — אותו JWT, ובנוסף המייל נמצא בטבלת admins (או ב-ADMIN_EMAILS).
 ...
 *
 * עד 14.9 הבדיקה כאן נשענה רק על ADMIN_EMAILS ב-Vercel, בעוד שה-RLS נשען על
 * הטבלה. שני מקורות לאותה עובדה נפרדו: סרגל הלקוחות (RLS) נטען, וכל קריאות
 * ה-API של האדמין (גישות, לוג כניסות, נתוני דשבורד) חזרו ריקות. ADMIN_EMAILS
 * מוגדר כ-Secret ואי אפשר לקרוא אותו בחזרה, כך שלא היה אפילו איך לבדוק מה
 * נשמר בו. המשתנה עדיין מכובד כתוספת, אבל כבר לא נדרש.
 *
...
export async function requireProjectAccess(req, projectId, bodyToken = null) {
  ...
  const user = await getUser(req, bodyToken)
  if (!user) return { ok: false, res: unauthorized() }
  if (await isAdminEmail(user.email)) return { ok: true, user, internal: false }
