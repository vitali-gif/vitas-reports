...
// ---------- 📊 מנהל קמפיינים — Hour of day (שעות חלשות → הפחתת תקציב) ----------

export function buildHourOfDayRec(hourlyLeads, totalLids) {
  if (!Array.isArray(hourlyLeads) || hourlyLeads.length !== 24) return null
  const total = hourlyLeads.reduce((a, b) => a + b, 0)
  if (total < 30) return null  // need enough leads to trust an hourly pattern
  // Paid-delivery window we'd consider for dayparting: 08:00–22:59
  const winHours = []; for (let h = 8; h <= 22; h++) winHours.push(h)
  const winTotal = winHours.reduce((s, h) => s + (hourlyLeads[h] || 0), 0)
  if (winTotal < 20) return null
  const avg = winTotal / winHours.length
  if (avg <= 0) return null
  // Quiet hours = within the window, leads well below average (< 40% of avg)
  const quiet = winHours.filter(h => (hourlyLeads[h] || 0) < avg * 0.4)
  if (quiet.length < 2) return null
  const quietLeads = quiet.reduce((s, h) => s + (hourlyLeads[h] || 0), 0)
  const quietShare = (quietLeads / total) * 100
  if (quietShare > 12) return null  // only flag genuinely low-yield hours
  let peak = winHours[0]; for (const h of winHours) if ((hourlyLeads[h] || 0) > (hourlyLeads[peak] || 0)) peak = h
  // group consecutive quiet hours into ranges
  const sorted = [...quiet].sort((a, b) => a - b)
  const ranges = []; let st = sorted[0], pr = sorted[0]
  for (let i = 1; i < sorted.length; i++) { if (sorted[i] === pr + 1) pr = sorted[i]; else { ranges.push([st, pr]); st = sorted[i]; pr = sorted[i] } }
  ranges.push([st, pr])
  const fh = (h) => String(h).padStart(2, '0') + ':00'
  const rangesStr = ranges.map(([a, b]) => a === b ? fh(a) : `${fh(a)}–${fh(b + 1)}`).join(', ')
  return {
    role: 'campaign_manager',
    type: 'hour_of_day',
    dedupKey: 'hour_low',
    icon: '🕐',
    title: 'תזמון תקציב — שעות חלשות בלידים',
    body: [
      `שעת השיא ללידים: ${fh(peak)} (${hourlyLeads[peak]} לידים).`,
      `בשעות ${rangesStr} נכנסים רק ${Math.round(quietShare)}% מהלידים — נמוך משמעותית מהממוצע השעתי.`,
      `סביר שתקציב הפרסום בשעות הללו פחות יעיל.`,
    ],
    suggestion: `הגדר תזמון מודעות (dayparting): הפחת או השהה תקציב בשעות ${rangesStr}, והעבר אותו לשעות החמות (סביב ${fh(peak)}).`,
    prediction: {
      label: 'חיסכון פוטנציאלי',
      value: `הפניית תקציב משעות עם ${Math.round(quietShare)}% מהלידים לשעות החמות`,
      detail: 'צמצום בזבוז בשעות החלשות צפוי לשפר עלות לליד',
    },
    measure: [
      `שיעור הלידים בשעות ${rangesStr} — האם ירד אחרי שינוי התזמון?`,
      `עלות לליד כוללת — האם השתפרה?`,
    ],
    baseline: { metric: 'hour_low', share: quietShare, quietHours: quiet, peakHour: peak, total },
    target: { metric: 'hour_low_reduced', shareTarget: Math.max(0, quietShare - 3) },
  }
}
...
export function buildRecommendations(input) {
  ...
  const r3b = buildHourOfDayRec(hourlyLeadStats, totalLids); if (r3b) recs.push(r3b)
...
export function compareImpact(task, currentInput) {
  ...
  try {
    switch (type) {
      ...
      case 'hour_of_day': {
        // Quiet-hours lead share should DROP after shifting budget to hot hours
        const hours = baseline.quietHours || []
        const arr = currentInput.hourlyLeadStats || []
        const tot = Array.isArray(arr) ? arr.reduce((a, b) => a + b, 0) : 0
        const q = hours.reduce((s2, h) => s2 + (arr[h] || 0), 0)
        baselineVal = baseline.share || 0
        currentVal = tot > 0 ? (q / tot) * 100 : 0
        betterIsHigher = false
        label = 'שיעור לידים בשעות החלשות'
        break
      }
      case 'top_objection': {
