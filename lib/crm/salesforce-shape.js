/**
 * lib/crm/salesforce-shape.js — קוד העיצוב של הסיכום, שנוצר *אוטומטית* מ-app/api/salesforce/fetch/route.js
 * (הקטע מ-"response time from LeadHistory" עד summary) על ידי scratch/gen-sf-shape.cjs.
 * אל תערוך ידנית: כשה-route משתנה, מריצים את המחולל מחדש. השאילתות הוחלפו ב-E (התוצאות המחוקות
 * מ-lib/crm/salesforce-summary.js); כל השאר זהה שורה-בשורה.
 */
import { STAGE_PAID, STAGE_QUOTE, STAGE_LOST, STATUS_NOSHOW, STATUS_SCHEDULED, STATUS_ARRIVED, STATUS_MEET, DOW, israelOffsetHours, businessHoursBetween, num, r0, pairs } from './salesforce-common.js'
import { SF_SCHEMA_VERSION } from './schema-version.js'
const CHAIN = 'קלוס'

export function shapeSalesforceSummary(E, since) {
  const { totalLeads, convertedLeads, meetingLeads, meetingPeriodCnt, noShowPeriodCnt, byStatusR, byBranchR, bySourceR, reasonsR, competitorsR,
    oppStageR, oppBranchR, salesmenR, purposeR, productsR, lossReasonR, lossByBranchR, unqualByBranchR, cohortStagesR,
    noShowR, arrivedBranchR, schedBranchR, mtgBranchR, stageBranchR, salesBranchR, prodBranchR, mtgHourR, mtgDayR, branchCohortR } = E
  ...
  try {
    const [oppNotesR, leadNotesR] = [E.oppNotesR, E.leadNotesR]
  ...
  try {
    const [cohortSmR, cohortProdR] = [E.cohortSmR, E.cohortProdR]
    const brSmC = {}
  ...
  let timingErr = null
  try {
    const [leadGridR, mtgDayBrR, bookDayR, mtgDayPaidR] = [E.leadGridR, E.mtgDayBrR, E.bookDayR, E.mtgDayPaidR]
    for (const r of (leadGridR || [])) {
      const h = Number(r.h) || 0, d = Number(r.d) || 1, c = r.c
      const localH = ((h + OFF) % 24 + 24) % 24
  ...
  const timing = { branches: timingBranches, data: timingData, respLabels: RESP_LABELS, days: DOW, _err: timingErr }
  ...
  try {
    const [srcStatusR, srcOppR] = [E.srcStatusR, E.srcOppR]
  ...
  return { xlsxRows, summary, totalLeads, opportunities, paid, dealValue, quotesTotal }
}
