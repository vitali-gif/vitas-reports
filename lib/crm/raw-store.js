...
export function extIdOf(entity, row) {
  for (const k of ID_CANDIDATES[entity] || []) {
    const v = row?.[k]
...
export async function loadRawRecords(sb, projectId, crmType, { pageSize = 1000 } = {}) {
  ...
  const counts = { clients: 0, tasks: 0, price_offers: 0, contracts: 0 }
  ...
  for (;;) {
    ...
    for (const r of data || []) {
      ...
      if (k) { out[k].push(r.payload); counts[r.entity]++ }
  ...
  return { ...out, counts, fetchedAt, total }
}
...
export async function loadCompactSnapshot(sb, projectId, crmType) {
  ...
  return { ...out, counts, fetchedAt: data.source_fetched_at || data.built_at, builtAt: data.built_at, total, compact: true }
}

/** רק המטא של התמונה הדחוסה (built_at, counts) — קריאה זעירה, לבדיקת מטמון לפני טעינת ה-payload. */
export async function loadCompactMeta(sb, projectId, crmType) {
  const { data, error } = await sb.from('crm_compact')
    .select('counts, source_fetched_at, built_at')
    .eq('project_id', projectId).eq('crm_type', crmType).maybeSingle()
