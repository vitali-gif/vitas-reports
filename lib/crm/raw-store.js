...
export async function upsertRawRecords(sb, projectId, crmType, entity, rows, { batch = 250, retries = 3 } = {}) {
  ...
  const all = [...byId.entries()].sort((a, b) => (a[0] < b[0] ? -1 : a[0] > b[0] ? 1 : 0))
    .map(([ext_id, payload]) => ({ project_id: projectId, crm_type: crmType, entity, ext_id, payload, fetched_at: fetchedAt }))
  let retried = 0
  for (let i = 0; i < all.length; i += batch) {
    const slice = all.slice(i, i + batch)
    for (let attempt = 0; ; attempt++) {
      const { error } = await sb.from('crm_raw').upsert(slice, { onConflict: 'project_id,crm_type,entity,ext_id' })
      if (!error) break
  ...
  return { count: all.length, idField, fallback, retried }
}
