/**
 * lib/crm/bmby-summary.js — חישוב סיכום ה-CRM של BMBY מרשומות גולמיות.
 *
 * הופרד מ-app/api/bmby/fetch/route.js ב-16.9.2026 (שלב 0 של docs/daily-ranges-plan.md).
 * ה-route מושך את הרשומות (clients / tasks / price_offers / contracts) מ-BMBY;
 * הפונקציה הזאת טהורה: אותן רשומות + אותו טווח + אותו "היום" → אותו סיכום, בכל מקום
 * שמריצים אותה (ה-route, ובקרוב נקודת הקצה לטווחים מותאמים מתוך תמונת מצב שמורה).
 *
 * הקוד הועבר כמו שהוא, בכוונה — כדי שהפלט יהיה זהה בית לבית לדוחות השמורים.
 * שני שינויים בלבד: `now` ניתן להזרקה (במקום new Date()), ו-CRM_SCHEMA_VERSION מיובא.
 */
import { normalizeCity } from '../city-normalize.js'
import { businessMinutesBetween } from '../business-hours.js'
import { CRM_SCHEMA_VERSION } from './schema-version.js'
...
export function decodeBmbyEntities(str) {
  if (!str) return str
  return String(str)
...
export function translateLivingStatus(raw) {
...
/**
 * צורת השורה בטבלת reports — data / summary / row_count — מתוך תוצאת החישוב.
 * מקום אחד לסדר המפתחות, כדי ששורה שנכתבה מה-route ושורה שחושבה מתמונת מצב
 * יהיו זהות בית לבית (זה מה שמאפשר להשוות ביניהן).
 */
export function toReportRow(R) {
  return {
    data: R.xlsxRows,
    summary: { ...R.totals, sources: R.sources, crmRepRows: R.crmReportRows, responseTimeStats: R.responseTimeStats, dayOfWeekStats: R.dayOfWeekStats, meetingDayOfWeek: R.meetingDayOfWeek, hourlyApptStats: R.hourlyApptStats, hourlyLeadStats: R.hourlyLeadStats, hourlyContactStats: R.hourlyContactStats, hourlyContactMeeting: R.hourlyContactMeeting, noAnswerContactHour: R.noAnswerContactHour, namedLeads: R.namedLeads, completedMeetings: R._completedMeetings, adBreakdown: R.adBreakdown, schemaVersion: CRM_SCHEMA_VERSION },
    row_count: R.aprilLids.length + R.registrationsInRange.length + R.contractsSignedInRange.length,
  }
}

/**
 * @param {{clients:any[], tasks:any[], prices:any[], contracts:any[]}} raw  רשומות BMBY גולמיות
 * @param {{since:string, until:string, monthKey:string, now?:Date}} period  YYYY-MM-DD, מפתח התקופה, ושעון
 * @returns {object} כל תוצרי החישוב: xlsxRows, crmReportRows, totals, sources, responseTimeStats, namedLeads, ... ו-_skippedBroken
 */
export function computeBmbySummary({ clients, tasks, prices, contracts }, { since, until, monthKey, now } = {}) {
  const m = monthKey
  now = now || new Date()
  ...
  const _todayStr = new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Jerusalem' }).format(now)
  ...
  for (const lid of aprilLids) {
    ...
    if (followups.length === 0) {
      // owner = בעל הליד ב-BMBY (אין פעולה אנושית, אז אין "נציג ראשון") — לעמודת "טרם התקיימה שיחה" לפי נציג
      const _ownerId = String(lid.user_id || lid.create_user_id || '')
      responseTimes.push({ source, responseMinutes: null, noResponse: true, owner: (_ownerId && userIdToName.get(_ownerId)) || null })
      continue
    }
  ...
  // לידים שטרם קיבלו מענה אנושי, לפי מקור ולפי בעל הליד (טבלאות "זמני תגובה" בדשבורד)
  const noResponseBySource = {}
  const noResponseByUser = {}
  ...
  totals.leadsToHandle = clients.filter(c => String(c.client_stage || '') === 'not_handeled').length  // "לידים לטיפול" = BMBY's live not_handeled queue (all-time, matches BMBY exactly). ONCE=1/REHAVIA=1/HIPARK=0 confirmed 2026-08-31
