...
export async function computeHealth(sb) {
  ...
  for (const p of projects) {
    ...
    const usesCrm = rs.some(r => r.source === 'crm') || crmMonthByProject.has(p.id)
    ...
    if (usesCrm) {
      ...
      // הרעננות נמדדת על הכתיבה ה-CRM הטרייה ביותר (מפתחות הטווח נוצרים מחדש כל
      // ריצה) — היא העדות היחידה הצפיפה לכך שהקרון רץ, כי created_at של החודש קפוא.
      const crmFresh = newest(rs, r => r.source === 'crm')
      const ageM = hoursAgo(crmFresh && crmFresh.created_at)
      checks.push({ label: 'CRM חודש נוכחי — עדכני',
        status: !crmMonth ? 'red' : (!crmFresh ? 'red' : (active && ageM > 6 ? 'yellow' : 'green')),
        detail: !crmMonth ? 'אין שורת חודש' : (crmFresh ? `הקרון רץ לפני ${ageM.toFixed(1)} ש׳` : 'הקרון לא רץ לאחרונה') })
