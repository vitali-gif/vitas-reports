...
export async function computeHealth(sb) {
  ...
  // 2026-09-19 — ⚠️ בלי summary. השאילתה הזאת החזירה 7.4MB של JSON בכל ריצה שעתית (180 שורות,
  // ממוצע 41KB, מקסימום 146KB), אבל summary נחוץ רק לשורות ה-CRM: repRowsLen קורא crmRepRows
  // ו-crmType נקרא משורת החודש או מטווח. 4.6MB מתוך זה נזרקו בלי שנגענו בהם. ה-route השעתי
  // רץ עם maxDuration=60 ו-cron-job.org חותך ב-30 שניות, אז כל מגה־בייט מיותר מתקרב לתקרה.
  const { data: lightRows } = await sb.from('reports')
    .select('project_id, source, month, created_at')
    .gte('created_at', sinceIso)
  // summary רק ל-CRM (60 שורות, 2.8MB) — משם מגיעים crmRepRows ו-crmType.
  const { data: crmRows } = await sb.from('reports')
    .select('project_id, month, created_at, summary')
    .eq('source', 'crm')
    .gte('created_at', sinceIso)
  const summaryByKey = new Map((crmRows || []).map(r => [`${r.project_id}|${r.month}`, r.summary]))
  const rows = (lightRows || []).map(r => (
    r.source === 'crm' ? { ...r, summary: summaryByKey.get(`${r.project_id}|${r.month}`) || null } : r
  ))
...
export function renderHealthEmail(h, { digest }) {
  ...
  return `<div style="font-family:Arial,sans-serif;direction:rtl;text-align:right;max-width:640px">
    ${head}${projBlocks}
    <p style="color:#888;font-size:12px;margin-top:16px">Tovno by Vitas · חיישני בריאות נתונים · בדיקת נתונים משותפת ללקוח ולאדמין</p></div>`
}
