...
import { requireProjectPlan, requireUser, isInternalCall } from '../auth'

/**
 * ישיבות שיווק הוא פיצ'ר PRO (מיגרציה 020). השער כאן הוא האכיפה היחידה — התגית
 * "PRO" על הכפתור בדשבורד היא שילוט בלבד, וכל מי שיודע לנחש כתובת יכול לפנות
 * ל-/api/meetings ישירות. אדמין וקרון פנימי עוברים; לקוח basic מקבל 403 עם
 * code: 'PLAN_REQUIRED', כדי שהדפדפן יציג מסך שדרוג ולא "אין גישה".
 */
...
export async function requireMeeting(req, meetingId) {
  ...
  const gate = await requireProjectPlan(req, data.project_id, REQUIRED_PLAN)
...
export async function requireProject(req, projectId) {
  ...
  const gate = await requireProjectPlan(req, projectId, REQUIRED_PLAN)
  if (!gate.ok) return { ok: false, res: gate.res }
  let sb
  try { sb = meetingsClient() } catch (e) { return { ok: false, res: configError(e) } }
